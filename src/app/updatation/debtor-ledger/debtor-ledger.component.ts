...
import {
  AfterViewInit, Component, HostListener, OnInit, ViewChild, ElementRef,
} from '@angular/core';
import {
...
// import { CreditorsdashboardComponent } from '../creditorsdashboard/creditorsdashboard.component';
...
@Component({
  ...
  styleUrls: ['./debtor-ledger.component.scss'],
  providers: [{
    provide: DateAdapter,
...
export class DebtorLedgerComponent implements OnInit, AfterViewInit {
  // @ViewChild(CreditorsdashboardComponent, { static: false }) child1: CreditorsdashboardComponent|any;
    ...
    if (response.length > 0) {
      response.forEach((element:any) => {
        ...
        if (element.currentyear == 1) {
          ...
          this.DebtorForm.get('tdate').setValue(formatDate(new Date(), 'yyyy-MM-ddTHH:mm:ss', 'en'));
        }
        this.DashboardValue();
      });
    }
...
ViewOnChange(event:any) {
 ...
 proceed() {
   console.log(this.DebtorForm.controls);

   console.log(this.DebtorForm.valid, this.DebtorForm.value);
