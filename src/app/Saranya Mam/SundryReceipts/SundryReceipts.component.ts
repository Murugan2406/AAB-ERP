...
@Component({
  selector: 'app-SundryReceipts',
  templateUrl: './SundryReceipts.component.html',
  styleUrls: ['./SundryReceipts.component.scss'],
})
export class SundryReceiptsComponent implements OnInit {
  ...
  SRTitleHeader: String = 'Sundry Receipts';
  ...
  onfromDateSelect(_event: any) {
    this.minTemp = _event.target.value;
    if (_event.key === 'Enter' || _event.key === 'Tab') {
      setTimeout(() => {
  ...
  selectViewReceiptRoute(_event: any) {
    if (_event.value) {
      setTimeout(() => {
  ...
  selectViewReceiptMode(_event: any) {
    if (_event.value) {
      setTimeout(() => {
  ...
  selectStatus(_event: any) {
    this.statuss = _event.value;
    if (_event.value) {
      setTimeout(() => {
  ...
  selectApprovalStatus(_event: any) {
    this.statuss = _event.value;
    if (_event.value) {
      if (_event.value === 'AUTHORIZED') {
        this.status = 'APPROVED';
      } else {
        this.status = _event.value;
      }
      this.filterApprovalSR();
      setTimeout(() => {
        document.getElementById('button')?.focus();
  ...
  selectTranType(event: any) {
    if (event.value) {
      ...
      if (this.tranType === 'RecVcr_SunRec') {
        this.SRTitleHeader = 'Sundry Receipts Approval';
        this.filterApprovalSR();
      } else if (this.tranType === 'RecVcr_SunRec_Rev') {
        this.SRTitleHeader = 'Sundry Receipts Reverse Approval';
        this.filterApprovalSR();
      }
    }
  }
  ...
  selectViewTranType(event: any) {
    if (event.value) {
      this.tranType = event.value;
      if (this.tranType === 'RecVcr_SunRec') {
        this.SRTitleHeader = 'Sundry Receipts';
        this.filterViewSR();
      } else if (this.tranType === 'RecVcr_SunRec_Rev') {
        this.SRTitleHeader = 'Sundry Receipts Reversal Request';
        this.filterViewSR();
      }
    }
    setTimeout(() => {
  ...
  displayFb2 = (option) => (option && option.FbName ? option.FbName : '');
